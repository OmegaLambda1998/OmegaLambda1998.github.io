...
function hfun_getrepos()
    ...
    for repo in repos
        ...
        if repo.has_pages && (repo.name != "OmegaLambda1998.github.io")
            write(io, """<a href="/$(repo.name)/" target="_blank">""")
        else
            write(io, """<a href="$(repo.html_url)" target="_blank">""")
        end
        write(io, """<div class="cardimg"><img src="$(get_default_img())"></div>""")
        write(io, """<div class="cardcontainer">""")
        write(io, """<div class="cardtitle">$(repo.name)<sup><i class="fa fa-external-link" aria-hidden="true"></i></sup></div>""")
...
function hfun_gettalks()
    ...
    for f in files
        if splitext(f)[end] in [".pdf"]
            ...
            write(io, """<div class="card">""")
            write(io, """<a href="/assets/archive/$(basename(f))" target="_blank">""")
            write(io, """<div class="cardimg"><img src="$(cardimg)"></div>""")
            write(io, """<div class="cardcontainer">""")
            write(io, """<div class="cardtitle">$(name)<sup><i class="fa fa-external-link" aria-hidden="true"></i></sup></div>""")
